...
Или используйте скрипт быстрого запуска с предварительно настроенными параметрами:
...
Приложение поддерживает следующие параметры:

```sh
go run cmd/app/main.go [опции]

Опции:
  --messages=N     Количество сообщений для получения из каждого диалога (по умолчанию: 5)
  --interval=N     Интервал обновления в секундах (по умолчанию: 60)
  --formats=F      Форматы вывода через запятую: json,markdown,structured,byuser (по умолчанию: json,markdown)
  --debug          Включить режим отладки с подробным выводом информации
```

Примеры использования:

```sh
# Получение 10 сообщений с интервалом обновления 2 минуты
go run cmd/app/main.go --messages=10 --interval=120

# Сохранение только в JSON-формате с включенным режимом отладки
go run cmd/app/main.go --formats=json --debug
```
...
├── start.sh              # Скрипт для быстрого запуска с параметрами
├── go.mod
├── go.sum
└── README.md
```
...
7. Повторяет процесс с заданным интервалом (настраивается через параметр --interval)
...
- Веб-интерфейс для просмотра и управления сообщениями
